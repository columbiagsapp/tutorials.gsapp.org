define(['jquery', 'underscore-min', 'backbone-min', 'drupalbackbone'],
    function($, _, Backbone, Drupal, Lesson) {
		  ...
		  addLesson: function(){
		    ...
		    var resp = l.save({}, {
		      success: function(model, response, options){
		        ...
		        if(Global.LessonsCollectionView[weekID] == undefined){
		          Global.LessonsCollection[weekID] = new LessonCollectionPrototype();
		          Global.LessonsCollection[weekID].reset();

		          var theEL = '#node-' + weekID + ' .lessons-list-el';

		          Global.LessonsCollectionView[weekID] = new LessonCollectionViewPrototype({
		            collection: Global.LessonsCollection[weekID],
		            templateSelector: '#lesson-list',
		            renderer: 'underscore',
		            el: theEL,
		            ItemView: Lesson.LessonView,
		            itemParent: '.lesson-list-container'
		          });

		          Global.LessonsCollectionView[weekID].render();
		        }

		        var newLessonView = Global.LessonsCollectionView[weekID].addOne(l);
